export function semanticValidator(question, issues) {
    // Rule 1: MC content must have mc answer format
    if (question.content.type === "mc_single" &&
    ...
    if (question.content.type === "number_input" &&
        question.answer.format === "mc") {
        issues.push({
            type: "semantic",
            severity: "error",
    ...
    if (question.pedagogy.difficulty_declared >= 9) {
        issues.push({
            type: "semantic",
            severity: "warning",
            message: "Very high declared difficulty — review recommended"
        });
    }
}
