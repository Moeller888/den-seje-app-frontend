-- Fix column reference ambiguity in get_teacher_visibility.
-- In Postgres 17, the RETURNS TABLE output column "student_id" is in scope
-- as a PL/pgSQL variable inside RETURN QUERY, causing an ambiguity error
-- when the agg CTE references ranked.student_id without a table qualifier.
-- Fix: alias the ranked CTE as "r" and qualify all references.
