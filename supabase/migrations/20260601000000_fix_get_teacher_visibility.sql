-- Fix get_teacher_visibility: column reference ambiguity + SMALLINT type mismatch.
--
-- Root cause (Postgres 17): RETURNS TABLE output column "student_id" is in scope
-- as a PL/pgSQL variable inside RETURN QUERY, making unqualified "student_id"
-- references in the agg CTE ambiguous. Fix: alias ranked as "r" and qualify all
-- column references. Also casts the current_band fallback literal to SMALLINT to
-- match the declared return type (COALESCE promotes integer literals to INTEGER).
--
-- This regression was introduced in the Section 92 migration (DROP + CREATE).
-- The original function pre-Section 92 was never exercised by automated tests,
-- so the breakage was latent until Section 97 added teacher E2E tests.
