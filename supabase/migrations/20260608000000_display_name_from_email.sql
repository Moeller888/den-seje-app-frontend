-- Section 142A: Use email local-part as student display name in the teacher
-- visibility overview instead of the generic full_name fallback ("Elev").
--
-- Before: COALESCE(NULLIF(TRIM(p.full_name), ''), 'Elev')
-- After:  COALESCE(NULLIF(split_part(u.email, '@', 1), ''), 'Elev')
--
-- Adds a JOIN to auth.users (same pattern used by get_my_students RPC).
-- Fallback to 'Elev' only when email is NULL or empty — should never happen
-- in practice since Supabase requires email for every auth.users row.
