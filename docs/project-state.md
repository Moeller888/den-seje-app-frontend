...
| **D-043** | **Phase-2 base RECOVERY basis reset — iter7 invalidated; `recovery-base-v1-blankface.png` adopted (2026-07-05).** The **iter7** base candidate is **visually INVALIDATED** by a corrective audit: a **skin-colored bust/chest-plate artifact** (a wide flat skin oval on the shirt, no real chin→neck→collar) — a **structural anatomy failure**, not the "minor finish" issue the earlier review recorded (the §2 "proportions PASS" relied on per-row width metrics that missed the shape; pale-on-white composites camouflaged it). The whole deterministic-carve line (iter4→iter7) is invalidated for the same reason, and the **owner-countersigned iter7 164B.3 CONDITIONAL PASS is WITHDRAWN/SUPERSEDED**. **Decision:** adopt **`source-master(3).png`** (preserved in-repo as `assets/avatar/reference/recovery-base-v1-blankface.png`, 1024×1536 RGBA) as the **new Phase-2 recovery base geometry basis** — it fixes the failure (correct bald scalp, intact ears, proper head→neck→collar, no bust-plate, cleanly blank face, clean alpha). **This intentionally supersedes the "frozen `Northstar Master.png` only" (D-032) geometry assumption *for the recovery path*** — the basis is a **separate re-rendering**, **not pixel-registered to the frozen Master** (different scale/position). **The basis is NOT a runtime asset, NOT promoted, and NOT yet passed:** its outfit is **not neutral** (original green sweater+star/cargo/green shoes → must be neutralized, D-029/D-022), it must be **re-registered** to the Master anchor space (or the register re-datumed), and it needs a **fresh 164B.3 review + composited sign-off** before Gate 2 can be satisfied. **Gate 2 REOPENED / under recovery; Gate 3 PAUSED; hair/eyes/face tooling still useful but their outputs are NOT approved layers against the new basis. `AVATAR_R2` stays `false`; no runtime/manifest/promotion change.** Full note: [`167a-phase2-base-recovery-decision.md`](./167a-phase2-base-recovery-decision.md). (2026-07-05) |
...
- **Phase-2 implementation gates — GATES 1 + 4 SATISFIED; GATE 2 REOPENED / UNDER RECOVERY (2026-07-05);
  ...
  (Re-check registration once the recovery basis is aligned.)
  ...
  **VISUALLY INVALIDATED** (skin-colored **bust/chest-plate** artifact = structural anatomy failure);
  ...
  marked invalidated). **New recovery basis adopted (D-043):** `recovery-base-v1-blankface.png`
  (`source-master(3).png`, preserved at `assets/avatar/reference/`) — fixes the anatomy (correct bald
  scalp, ears, head→neck→collar, no bust-plate, blank face). **NOT passed yet:** it needs
  **re-registration** to the Master anchor space, **outfit neutralization** (still green sweater+star/
  cargo/green shoes → plain grey, D-029/D-022), and a **fresh 164B.3 review + composited sign-off**.
