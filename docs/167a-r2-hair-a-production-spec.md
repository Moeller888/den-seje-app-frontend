# 167A — Option A production spec: the six missing R2 hairstyles

**Status:** `SPEC_READY — PRODUCTION_METHOD_UNDECIDED`.
...
**Decides nothing about *who or what* draws the artwork** — see §2, which is the open question.
...
is the reason §2 is open)**, D-032 (Master = geometry), D-059 (judge at render scale), D-071
...
exist is the artwork — and, as §2 sets out, the authorisation to make it.

## 2. The blocker: no production method is authorised

**This is the finding that gates everything below, and it corrects the audit.**
...
**So option A is authorised by neither route today:** not by D-033 (no painter exists), and not by
D-042 (masked decomposition only covers what the Master already contains). It needs a fresh owner
decision naming a production method and the identity-lock that goes with it. The plausible routes,
stated without recommending one:

| Route | What it would require |
|---|---|
| **Human illustrator** | The original D-033 method. Blocked on availability, which is why D-042 exists. |
| **Extend D-042 to new silhouettes** | A new register entry that permits AI to author hairstyle silhouettes the Master does not contain, with an identity-lock story that replaces "masking" — because masking is exactly what cannot apply to a shape that is not there. The R-6 drift risk D-032/D-033 measured applies in full. |
| **Derive from the C2 vectors** | The seven C2 hairstyles already exist as authored SVG path data. Converting them to the R2 painted style is neither pure decomposition nor free generation, and would need its own decision. Note this is close to option B, which the owner declined on product grounds. |

**Nothing below depends on which route is chosen.** The canvas, the geometry, the asset contract
and the gates are properties of the runtime, not of the hand that draws.

## 3. Scope: six assets or seven?
...
Two consistent readings, and the owner has to pick one:

- **Seven assets.** Every selectable style gets its own raster; `hair-northstar-v1` stays as the
  fallback for an unknown or legacy value. Honest, and one more asset than the audit assumed.
- **Six assets + a nomination.** The owner nominates which of the seven the existing asset shall
  represent, accepting that it measurably is not that style. This is the same nomination problem
  that removed option C's footing.

**Open — recorded, not decided here.**
...
1. **§2 — the production method.** Nothing can be drawn until this is answered.
2. **§3 — six assets or seven**, and if six, which style the existing asset is nominated for.
3. Whether `long` and `ponytail` ship in the first slice at all, given §7's last point.
