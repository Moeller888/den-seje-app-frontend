# 167A Phase-2 — Base Recovery Decision (iter7 invalidated; new basis adopted)

**Status: DECISION — recorded 2026-07-05. Owner-directed art-basis reset for Phase-2 recovery.**
...
_Register entry: **D-043** in [`project-state.md`](./project-state.md). Supersedes the Gate-2
...
3. **`source-master(3).png` is ADOPTED as the new Phase-2 recovery base geometry basis.** Preserved
   in-repo as a **review/reference artifact** at
   [`assets/avatar/reference/recovery-base-v1-blankface.png`](../assets/avatar/reference/recovery-base-v1-blankface.png)
   (1024×1536 RGBA, clean transparent bg).
4. **This intentionally supersedes the prior "frozen `Northstar Master.png` only" (D-032) geometry
   assumption *for the failed recovery path*.** The recovery basis is a **separate re-rendering** of the
   same character — it is **not pixel-registered to the frozen Master** (different scale + vertical
   position; misregistration confirmed by overlay).
...
## 4. What this basis is NOT (still required before any Gate-2 pass)
...
- ⚠️ **Not registered to the frozen Master** — anchors, eye-box, masks and the Gate-3 layers are keyed
  to the frozen Master, so the basis must be **re-registered** (scale + translate) **or** the register
  formally re-datums to this render.
- ⚠️ **Still requires validation:** a fresh **164B.3 review** (against this basis) + composited visual
  sign-off before Gate 2 can be considered satisfied.
...
| **`recovery-base-v1-blankface.png`** | **ADOPTED as recovery basis — NOT yet passed** (needs registration + neutral outfit + 164B.3) |
| **Gate 3** | **PAUSED** (face/eyes/eyelid work stopped) |
| Gate-3 hair/eyes/face **tooling** | still useful *as tooling*; but their **outputs are NOT approved layers** against the new basis (Master-derived + iter7-composited → re-derive against the new basis after registration) |
| Gate 1 (eye-box sign-off) | unchanged ✅ (raster-path eye-box; re-check registration when the basis is aligned) |
| Gate 4 (WebP encoder) | unchanged ✅ |
| Gate 5 (composed sign-off) | open |

## 6. Recovery sequence (not started here — future, gated)

1. **Registration decision + execution:** align the recovery basis to the Master anchor space, or
   re-datum the register to this render. (Prerequisite for everything downstream.)
2. **Neutralize the outfit** (grey tee/trousers/shoes), identity-preserving (masked; D-042).
3. **Deterministic clean-up** (drop stray specks; verify alpha).
4. **Fresh 164B.3 review** of the corrected base + composited visual sign-off.
5. Only then re-derive/validate the Gate-3 layers (hair/eyes/face/eyelid) **against the new basis**.

**No AI generation, no ComfyUI, no new layer extraction, no promotion is authorized by this document —
it records the basis reset only.**
