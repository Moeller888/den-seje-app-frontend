...
| [167a-phase2-gate3-wp1-rereview.md](./167a-phase2-gate3-wp1-rereview.md) | 167A **Phase-2 Gate-3 WP1 re-review worksheet (2026-07-16)**: the punch-listed hair pair `hair-pl1-color.png` + `hair-pl2-luminance.png` re-audited against all four WP1 findings (`build-hair-wp1-rereview.mjs`) — gap 0, halo 130 (unchanged), silhouette aligned, map p50 200, alpha identity colour↔map 0 mismatch, chain re-run byte-identical (5/5). Includes the 2026-07-16 audit follow-ups (on-dark display, explicit 32 px decision, D-register decision). **✅ Owner-countersigned PASS (2026-07-16): WP1 conditional DISCHARGED — `hair-pl1-color.png` + `hair-pl2-luminance.png` = accepted Gate-3 hair candidate (z40).** Owner decisions: 32 px legibility ACCEPTED (palette property); Gate-3 decision record = countersigned worksheets (no D-entry). **Gate 3 stays gated; `AVATAR_R2` false; nothing promoted.** | Authoritative (review) |
| [167a-phase2-gate3-wp2-eyes-review.md](./167a-phase2-gate3-wp2-eyes-review.md) | 167A **Phase-2 Gate-3 WP2 eyes review worksheet (2026-07-16)**: neutral-luminance iris map (`build-eyes-wp2-refine.mjs`, D-031 multiply model, band [40,235]) on the WP0 iris disk — alpha identity 0, centroids ≤10 px vs 164S anchors, brown round-trip ≈ Master, 6-color preview tint sheet (preview set = proposal, no runtime `EYE_COLOR` tokens exist). Fixed layer untouched (D-021-complete). Spec conflict resolved: brief §4.4 governs — pupil stays in iris art. **✅ Owner-countersigned PASS (2026-07-16): WP2 CLEARED — `eyes-iris-wp2-luminance.png` + unchanged `eyes-neutral-fixed.png` = accepted Gate-3 eyes candidate (z4 pair).** Preview colour set stays a proposal (no runtime `EYE_COLOR` tokens ordered). **Gate 3 stays gated; `AVATAR_R2` false; nothing promoted.** | Authoritative (review) |
| [167a-phase2-gate3-wp3-face-review.md](./167a-phase2-gate3-wp3-face-review.md) | 167A **Phase-2 Gate-3 WP3 face×5 review worksheet (2026-07-16)**: face-neutral candidate audited vs brief §4.2 (`build-face-wp3-neutral.mjs` — 0 px outside regions/in eye boxes/eyes-layer overlap/skin-toned), evidence-based blush measurement (symmetric interior signal FOUND, 390 px — owner decision required), first complete Gate-3 stack composite, + the deterministic expression gate `validate-face-expression.mjs` (proven PASS/FAIL both ways). The 4 expression variants are D-042 art-producer scope (masked editing on the Master), NOT produced here. **✅ Owner-countersigned PASS (2026-07-16): `face-neutral-v1.png` = accepted Gate-3 face-neutral candidate (z3); BLUSH EXTRACTION ORDERED (G3-PL-B, next bounded task).** **Gate 3 stays gated; `AVATAR_R2` false; nothing promoted.** | Authoritative (review) |
| [167a-phase2-gate3-plb-blush-extraction.md](./167a-phase2-gate3-plb-blush-extraction.md) | 167A **Phase-2 Gate-3 PL-B blush-extraction worksheet (2026-07-16)**: deterministic multiply-component extraction of the Master's symmetric under-eye blush (`build-face-plb-blush.mjs` — `m = Master/D-057` per channel ≤1, smoothstep alpha 2–8 %, zones extended to y428) — 2,859 px, symmetry 0.87, tone-agnostic proof over medium + darkened skin. Packaging: separate multiply file (runtime wiring = later gated code step). **✅ Owner-countersigned PASS (2026-07-16): PL-B CLEARED — `face-blush-multiply-v1.png` = accepted Gate-3 blush component (pairs with face-neutral z3).** **Gate 3 stays gated; `AVATAR_R2` false; nothing promoted.** | Authoritative (review) |
| [167a-phase2-gate3-eyelid-decision.md](./167a-phase2-gate3-eyelid-decision.md) | 167A **Phase-2 Gate-3 eyelid decision worksheet (2026-07-16)**: measured evidence for brief §4.5's choice — interim CSS-ellipse (Option A, recommended) vs raster eyelid (Option B, D-042). "No off-eye flash" MEASURED: smallest covering lid = rx 57/ry 58 (+8 oversize) → 0 of 10,755 eye px uncovered; wiring numbers @160: centres (66.7,60.3)/(90.6,60.3), rx 8.91/ry 9.06; fill finding: engine `#EDB888` reads lighter than raster skin (`#FEC183` measured) — derive per-tone fill from the raster base at wiring. **✅ Owner-countersigned OPTION A CHOSEN (2026-07-16): interim CSS-ellipse re-positioned to the North Star eye box; measured wiring numbers binding; Option B stays available via D-042.** **Gate 3 stays gated; `AVATAR_R2` false; no runtime change until the separately gated wiring step.** | Authoritative (decision) |
| [167a-phase2-gate3-integration-composite.md](./167a-phase2-gate3-integration-composite.md) | 167A **Phase-2 Gate-3 integration-composite worksheet (2026-07-16)**: all countersigned candidates assembled (`build-gate3-integration-composite.mjs`) — full-stack head-zone coverage gap **0 px** (hard guard); Master fidelity identity path **p50 = 0** (median pixel exactly Master), runtime path p50 14 (D-031 approximation); §3 finding: runtime `brown` token renders darker than the Master's hair (multiply ceiling; token-faithful to shipped C2 — options a–d for the owner); 8-token hair matrix + 6-color iris matrix + 64/48/32 px legibility. **✅ Owner-countersigned PASS (2026-07-16): §3 option (a) chosen (token-brown default, token-faithful); THE GATE-3 DETERMINISTIC LAYER SET IS DECLARED COMPLETE** (base+blush+face+eyes+hair + eyelid Option-A parameters). Not promoted, no runtime wiring; expressions remain D-042 art-producer scope pending the external-image-API owner decision. **`AVATAR_R2` false.** | Authoritative (review) |
| [167a-phase2-gate3-neutral-asset-promotion.md](./167a-phase2-gate3-neutral-asset-promotion.md) | 167A **Phase-2 Gate-3 neutral asset promotion worksheet (PR A, 2026-07-18)**: the six countersigned neutral layers regenerated in an isolated worktree (all guards green, 6/6 byte-identical to the accepted candidates), encoded with the vendored libwebp 1.5.0 (`encode-webp.mjs`, q90/alpha_q100/-m6, ÷2 → 512×768, double-encode byte-identical) and **promoted as tracked files under `assets/avatar-r2/`** (39.4 KB total vs 350 KB budget). Binds U1 (measured Master-brown default iris `#A34A0F`), U2 (only `neutral`×`medium` — all else later C2 fail-soft) and U4 (blush name; §C row deferred to PR B). **✅ Owner-countersigned PASS (2026-07-18).** **NOT loaded by the app: no manifest entry, no runtime change, `AVATAR_R2` false, no expressions.** | Authoritative (review) |
